...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function loadLeads() {
      ...
      try {
        // Seed DB with demo leads if empty
        ...
        if (dbLeads.length > 0) {
          ...
          // Restore saved sequences, notes, and sent status from DB
  ...
  function handleFilterClick(status: "all" | LeadStatus) {
    ...
    if (next.length > 0 && !next.find((l) => l.id === selectedLead?.id)) {
      selectLead(next[0]);
    }
  }
  ...
  async function generateSequence() {
    ...
    try {
      const res = await fetch("/api/generate", {
        ...
        body: JSON.stringify({ lead: selectedLead, action: "generate", language: languageMode }),
      });

      const data = await res.json();

      if (res.status === 429) {
        const seconds = data.retryAfterSeconds ?? 30;
        startCountdown(seconds);
        showToast(`Quota limit — retry in ${seconds}s.`, "quota", seconds * 1000);
        return;
      }
      ...
      if (data.modelUsed && data.modelUsed !== "gemini-2.0-flash-lite") {
        showToast(`Note: Generated via ${data.modelUsed} (primary model at quota).`, "sync");
      }

      // Update lead in local state with savedSequence
      setLeads((prev) =>
        prev.map((l) =>
  ...
  async function regenerateTouch(tab: TouchType) {
    ...
    try {
      ...
      const label = tab === "touch1" ? "Email 1" : tab === "touch2" ? "Email 2" : tab === "linkedin" ? "LinkedIn" : "Call script";
  ...
  function toggleSent(touch: TouchType) {
    ...
    fetch("/api/leads", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id: selectedLead.id, sentStatus: updated }),
    }).catch(() => {});
  }
  ...
  function markAllSent() {
    ...
    setSentStatus((prev) => ({ ...prev, [selectedLead.id]: updated }));
    showToast("All 4 touches marked as sent.");
    fetch("/api/leads", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id: selectedLead.id, sentStatus: updated }),
  ...
  function updateNotes(value: string) {
    ...
    notesSaveTimer.current = setTimeout(() => {
      fetch("/api/leads", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: selectedLead.id, notes: value }),
      }).catch(() => {});
    }, 1200);
  }

  /* ── Save edited content on blur ────────────────────────────── */

  function saveEditedContent(ec: EditedContent) {
    fetch("/api/leads", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id: selectedLead.id, editedContent: ec }),
    }).catch(() => {});
  }
  ...
  function buildSequenceText(): string {
    ...
    return [
      ...
      `──────────────────────────────────`,
      `Subject: ${editedContent.touch1Subject}`, ``,
      editedContent.touch1Body, ``,
      `WHY: ${sequence.touch1.rationale}`, ``,
      `──────────────────────────────────`,
      `TOUCH 2 — ${sequence.touch2.timing}`,
      `──────────────────────────────────`,
      `Subject: ${editedContent.touch2Subject}`, ``,
      editedContent.touch2Body, ``,
      `WHY: ${sequence.touch2.rationale}`, ``,
      `──────────────────────────────────`,
      `LINKEDIN — ${sequence.linkedin.timing}`,
      `──────────────────────────────────`,
      editedContent.linkedinBody, ``,
      `WHY: ${sequence.linkedin.rationale}`, ``,
      `──────────────────────────────────`,
      `CALL SCRIPT — ${sequence.call.timing}`,
      `──────────────────────────────────`,
      `OPENING LINE: "${sequence.call.opener}"`, ``,
      editedContent.callScript, ``,
      `OBJECTION HANDLING:`,
      sequence.call.objections ?? "", ``,
      currentNote ? `NOTES:\n${currentNote}\n` : "",
      `═══ END OF SEQUENCE ═══`,
    ].join("\n");
  }
  ...
  async function pushToCRM() {
    ...
    // Also save latest edited content to DB during "push"
  ...
  async function confirmAddLead() {
    ...
    // Persist to DB and replace temp id with real cuid
    try {
      const res = await fetch("/api/leads", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...
          intentSignal: newLead.intentSignal,
          painPoint:    newLead.painPoint,
          status:       newLead.status,
          score:        newLead.score,
          website:      newLead.website,
          bestTime:     newLead.bestTime,
        }),
      });
      const data = await res.json();
      if (data.lead?.id) {
        ...
        setLeads((prev) =>
          prev.map((l) => (l.id === newLead.id ? { ...l, id: data.lead.id } : l))
        );
        setSelectedLead((prev) =>
          prev.id === newLead.id ? { ...prev, id: data.lead.id } : prev
        );
      }
    } catch {
      // silent — lead still works in session
    }
  }
    ...
    <div className={styles.container}>
      ...
      <div className={styles.sidebar}>
        ...
        <div className={styles.filterRow}>
          <div className={styles.filterTabs}>
            {(["all", "hot", "warm", "cold"] as const).map((f) => (
              <button
                ...
                className={filterStatus === f ? styles.filterTabActive : styles.filterTab}
              >
                {f.toUpperCase()}
              </button>
            ))}
          ...
          <button
            type="button" className={styles.sortBtn}
            onClick={() => setSortMode((m) => m === "score" ? "alpha" : "score")}
            title={sortMode === "score" ? "Sort A–Z" : "Sort by score"}
          >
        ...
        <div className={styles.leadList}>
          {isLoadingLeads ? (
            ...
            <div className={styles.noLeads}>
              {searchQuery ? `No results for "${searchQuery}"` : "No leads in this filter."}
            </div>
          ) : (
            filteredLeads.map((lead) => (
              <div
                ...
                className={cls(
                  ...
                  deleteConfirmId === lead.id && styles.leadCardDeleting
                )}
              >
                  ...
                  <div onClick={() => selectLead(lead)}>
                    <div className={styles.leadCardTop}>
                      ...
                      <div className={styles.leadCardTopRight}>
                        ...
                        <button
                          type="button" className={styles.deleteLeadBtn}
                    ...
                    <div className={styles.leadMeta}>
                      <div className={styles.scoreBar}>
                        <div className={styles.scoreBarFill} style={{ width: `${lead.score}%`, backgroundColor: getScoreColor(lead.score) }} />
                      </div>
      ...
      <div className={styles.main}>
        ...
        <div className={styles.intelligencePanel}>
          <div className={styles.intelligenceHeader}>
            ...
            <span className={getBadgeClass(selectedLead.status)}>
              {selectedLead.status === "hot" ? "🔴 HIGH PRIORITY" : selectedLead.status === "warm" ? "🟡 ACTIVE SIGNAL" : "⚪ MONITOR"}
            </span>
          </div>
          <div className={styles.signalRow}>
            <span className={styles.signalKey}>WHY NOW?</span>
            <span className={styles.signalVal}>{selectedLead.intentSignal}</span>
          </div>
          <div className={styles.signalRow}>
            <span className={styles.signalKey}>HYPOTHESIS</span>
            <span className={styles.signalVal}>{selectedLead.painPoint}</span>
          </div>
          <div className={styles.signalRow}>
            <span className={styles.signalKey}>BEST TIME</span>
            <span className={cls(styles.signalVal, styles.signalValAccent)}>
              {selectedLead.bestTime ?? "Tuesday 2:30 PM — Optimal post-lunch window for decision makers"}
            </span>
          </div>
        </div>
        ...
        <div className={styles.notesPanel}>
          ...
          {showNotes && (
            <textarea
              ...
              placeholder={`Add notes about ${selectedLead.contactName} or ${selectedLead.company}…`}
              value={currentNote}
        ...
        {complianceWarning && (
          <div className={styles.complianceBanner}>
            ...
            <span className={styles.complianceBannerText}>
              Compliance filter applied — non-compliant language was auto-sanitized per RBI guidelines.
            </span>
        ...
        {retryCountdown !== null && (
          <div className={styles.quotaBanner}>
            ...
            <span className={styles.quotaBannerText}>
              Gemini quota limit reached — auto-retry in <strong>{retryCountdown}s</strong>. All 3 models exhausted.
            </span>
          </div>
        )}
        ...
        <div className={styles.generateRow}>
          <button
            ...
            disabled={isGenerating || !!regenTouch || retryCountdown !== null}
            className={styles.generateBtn}
          >
            {isGenerating
              ? <><span className={styles.spinner} />ANALYZING & GENERATING…</>
              : retryCountdown !== null
          ...
          <div className={styles.sequenceArea}>
            ...
            <div className={styles.sequenceTimeline}>
              {TABS.map((tab, i) => (
                <div key={tab.key} className={styles.timelineStep}>
                  <div
                    className={cls(
                      styles.timelineNode,
                      activeTab === tab.key && styles.timelineNodeActive,
                      currentSent[tab.key] && styles.timelineNodeSent,
                      regenTouch === tab.key && styles.timelineNodeRegen
                    )}
                    onClick={() => !regenTouch && setActiveTab(tab.key)}
                  >
                    {regenTouch === tab.key
                      ? <span className={styles.spinner} style={{ borderColor: "rgba(0,212,170,.3)", borderTopColor: "var(--accent)" }} />
                      : <span className={styles.timelineIcon}>{tab.icon}</span>
                    }
              ...
              <div className={styles.timelineActions}>
                {!allSent
                  ? <button type="button" className={styles.markAllBtn} onClick={markAllSent} title="Mark all sent">✓ ALL</button>
                  : <button type="button" className={cls(styles.markAllBtn, styles.markAllBtnDone)} onClick={resetSent} title="Reset">↺ RESET</button>
                }
                <button type="button" className={downloadSuccess ? styles.exportBtnSuccess : styles.exportBtn} onClick={downloadSequence}>
                  {downloadSuccess ? "✓ SAVED" : "↓ TXT"}
                </button>
              </div>
            </div>
            ...
            <div className={styles.tabContent}>
              <div className={styles.tabHeader}>
                <div className={styles.tabHeaderLeft}>
                  <span className={styles.tabChannelBadge}>
                    {activeTab === "touch1" || activeTab === "touch2" ? "✉ EMAIL" : activeTab === "linkedin" ? "💼 LINKEDIN" : "📞 CALL SCRIPT"}
                  </span>
                  <span className={styles.tabTiming}>{sequence[activeTab].timing}</span>
                  {sentTouchCount > 0 && <span className={styles.progressPill}>{sentTouchCount}/4 SENT</span>}
                </div>
                <div className={styles.tabHeaderActions}>
                  <button
                    type="button" className={styles.regenBtn}
                    onClick={() => regenerateTouch(activeTab)}
                    disabled={!!regenTouch || isGenerating || retryCountdown !== null}
                  ...
                  <button
                    type="button"
                    className={copySuccess === activeTab ? styles.actionBtnCopied : styles.actionBtn}
                    onClick={() => copyToClipboard(getCopyText(activeTab), activeTab)}
                  >
                    {copySuccess === activeTab ? "✓ COPIED" : "COPY"}
                  </button>
                  <button
                    type="button"
                    className={currentSent[activeTab] ? styles.actionBtnSent : styles.actionBtn}
                    onClick={() => toggleSent(activeTab)}
                  >
                    {currentSent[activeTab] ? "✓ SENT" : "MARK SENT"}
                  </button>
              ...
              <div className={styles.rationaleBar}>
                ...
                <span className={styles.rationaleText}>{sequence[activeTab].rationale}</span>
              </div>
              ...
              {activeTab === "touch1" && (
                <>
                  <div className={styles.subjectRow}>
                    ...
                    <input className={styles.subjectInput} value={editedContent.touch1Subject}
                      onChange={(e) => setEditedContent((p) => p ? { ...p, touch1Subject: e.target.value } : p)}
                      onBlur={() => editedContent && saveEditedContent(editedContent)}
                    />
                  </div>
                  <textarea className={styles.draftTextarea} value={editedContent.touch1Body}
                    onChange={(e) => setEditedContent((p) => p ? { ...p, touch1Body: e.target.value } : p)}
                    onBlur={() => editedContent && saveEditedContent(editedContent)}
                  />
                  <div className={styles.wordCountRow}>
                    <span className={cls(styles.wordCount, wordCount(editedContent.touch1Body) > 160 && styles.wordCountOver)}>
                      {wordCount(editedContent.touch1Body)} words{wordCount(editedContent.touch1Body) > 160 ? " — over 160" : ""}
                    </span>
                  </div>
                </>
              )}
              ...
              {activeTab === "touch2" && (
                <>
                  <div className={styles.subjectRow}>
                    <span className={styles.fieldLabel}>SUBJECT</span>
                    <input className={styles.subjectInput} value={editedContent.touch2Subject}
                      onChange={(e) => setEditedContent((p) => p ? { ...p, touch2Subject: e.target.value } : p)}
                      onBlur={() => editedContent && saveEditedContent(editedContent)}
                    />
                  </div>
                  <textarea className={styles.draftTextarea} value={editedContent.touch2Body}
                    onChange={(e) => setEditedContent((p) => p ? { ...p, touch2Body: e.target.value } : p)}
                    onBlur={() => editedContent && saveEditedContent(editedContent)}
                  />
                  <div className={styles.wordCountRow}>
                    <span className={cls(styles.wordCount, wordCount(editedContent.touch2Body) > 140 && styles.wordCountOver)}>
                      {wordCount(editedContent.touch2Body)} words{wordCount(editedContent.touch2Body) > 140 ? " — over 140" : ""}
                    </span>
                  </div>
                </>
              )}
              ...
              {activeTab === "linkedin" && (
                <>
                  <div className={styles.charCountRow}>
                    ...
                    <span className={cls(styles.charCount, editedContent.linkedinBody.length > 300 && styles.charCountOver)}>
                      {editedContent.linkedinBody.length} / 300
                    </span>
                  </div>
                  <textarea className={cls(styles.draftTextarea, styles.linkedinTextarea)} value={editedContent.linkedinBody}
                    onChange={(e) => setEditedContent((p) => p ? { ...p, linkedinBody: e.target.value } : p)}
                    onBlur={() => editedContent && saveEditedContent(editedContent)}
                  />
                </>
              )}
              ...
              {activeTab === "call" && (
                <>
                  <div className={cls(styles.callBlock, styles.callBlockAccent)}>
                    ...
                    <p className={styles.callOpenerText}>&ldquo;{sequence.call.opener}&rdquo;</p>
                  </div>
                  <div className={styles.subjectRow}><span className={styles.fieldLabel}>FULL SCRIPT</span></div>
                  <textarea className={styles.draftTextarea} value={editedContent.callScript}
                    onChange={(e) => setEditedContent((p) => p ? { ...p, callScript: e.target.value } : p)}
                    onBlur={() => editedContent && saveEditedContent(editedContent)}
                  />
                  <div className={cls(styles.callBlock, styles.callBlockWarn)}>
                    ...
                    <p className={styles.objectionsText}>{sequence.call.objections}</p>
                  </div>
                </>
              )}
            </div>
        ...
        {!sequence && !isGenerating && (
          <div className={styles.emptyState}>
            ...
            <p className={styles.emptyText}>
              {selectedLead?.savedSequence
                ? "Sequence loaded from database."
                : `Generate a compliance-safe 4-touch sequence for ${selectedLead?.contactName} at ${selectedLead?.company}`
              }
            </p>
          </div>
        )}
            ...
            <div className={styles.modalBody}>
              <div className={styles.formRow}>
                <div className={styles.formGroup}>
                  ...
                  <input className={styles.formInput} placeholder="e.g. NeoBank India" value={newLeadForm.company} onChange={(e) => updateForm("company", e.target.value)} />
                </div>
                <div className={styles.formGroup}>
                  <label className={styles.formLabel}>CONTACT NAME *</label>
                  <input className={styles.formInput} placeholder="e.g. Siddharth Rao" value={newLeadForm.contactName} onChange={(e) => updateForm("contactName", e.target.value)} />
                </div>
              </div>
              <div className={styles.formRow}>
                <div className={styles.formGroup}>
                  <label className={styles.formLabel}>ROLE</label>
                  <input className={styles.formInput} placeholder="e.g. CTO" value={newLeadForm.role} onChange={(e) => updateForm("role", e.target.value)} />
                </div>
                <div className={styles.formGroup}>
                  <label className={styles.formLabel}>INDUSTRY</label>
                  <input className={styles.formInput} placeholder="e.g. Lending Tech" value={newLeadForm.industry} onChange={(e) => updateForm("industry", e.target.value)} />
                </div>
              </div>
              <div className={styles.formGroup}>
                <label className={styles.formLabel}>COMPANY SIZE</label>
                <input className={styles.formInput} placeholder="e.g. 50–200 employees" value={newLeadForm.companySize} onChange={(e) => updateForm("companySize", e.target.value)} />
              </div>
              <div className={styles.formGroup}>
                <label className={styles.formLabel}>RAW INTEL — paste a job posting, news article, or funding announcement *</label>
                <textarea className={styles.formTextarea} rows={4} placeholder="e.g. NeoBank India raised ₹60Cr and is hiring a 'Banking API Architect'…" value={newLeadForm.description} onChange={(e) => updateForm("description", e.target.value)} />
              </div>
              ...
              {detectedSignals && (
                <div className={styles.detectedSignals}>
                  <div className={styles.detectedHeader}>
                    ...
                    <span className={getBadgeClass(detectedSignals.status)}>
                      {(detectedSignals.status ?? "warm").toUpperCase()} · {detectedSignals.score ?? 70}/100
                    </span>
                  </div>
                  <div className={styles.signalRow}>
                    <span className={styles.signalKey}>INTENT</span>
                    <span className={styles.signalVal}>{detectedSignals.intentSignal}</span>
                  </div>
                  <div className={styles.signalRow}>
                    <span className={styles.signalKey}>PAIN POINT</span>
                    <span className={styles.signalVal}>{detectedSignals.painPoint}</span>
                  </div>
                  {detectedSignals.bestTime && (
                    <div className={styles.signalRow}>
                      <span className={styles.signalKey}>BEST TIME</span>
                      <span className={cls(styles.signalVal, styles.signalValAccent)}>{detectedSignals.bestTime}</span>
                    </div>
                  )}
                </div>
              )}
              ...
              <div className={styles.modalActions}>
                {detectedSignals ? (
                  <>
                    <button type="button" className={styles.redetectBtn} onClick={() => setDetectedSignals(null)}>RE-DETECT</button>
                    <button type="button" onClick={confirmAddLead} className={styles.confirmAddBtn}>+ ADD TO PIPELINE</button>
                  </>
                ) : (
                  <button type="button" onClick={detectSignals}
                    disabled={isDetecting || retryCountdown !== null || !newLeadForm.company || !newLeadForm.contactName || !newLeadForm.description}
      ...
      {showCrmModal && (
        <div className={styles.overlay} onClick={crmStep === "done" ? closeCrmModal : undefined}>
          <div className={cls(styles.modal, styles.crmModal)} onClick={(e) => e.stopPropagation()}>
            <div className={styles.modalHeader}>
              <span className={styles.modalTitle}>PUSH TO CRM — {crmLeadName}</span>
              {crmStep === "done" && <button type="button" className={styles.modalClose} onClick={closeCrmModal}>✕</button>}
            </div>
            <div className={styles.crmModalBody}>
              {CRM_STEPS.map(({ step, label }) => {
                ...
                return (
                  <div key={step} className={cls(styles.crmStepRow, active && styles.crmStepActive, done && styles.crmStepDone)}>
                    <div className={cls(styles.crmStepIcon, active && styles.crmStepIconActive, done && styles.crmStepIconDone)}>
                      {done ? "✓" : active ? <span className={styles.spinnerCrm} /> : "○"}
                    </div>
              ...
              {crmStep === "done" && (
                <div className={styles.crmSuccess}>
                  ...
                  <div className={styles.crmSuccessDetails}>
                    <span>{sentTouchCount}/4 touches recorded</span>
                    <span>·</span><span>Stage: Active Outreach</span>
                    <span>·</span><span>Owner: Blostem Sales</span>
                  </div>
      ...
      {toast && (
        <div
          ...
          className={cls(
            ...
            toast.type === "sync" && styles.syncToast,
            toast.type === "quota" && styles.quotaToast
          )}
        >
          {toast.type === "success" ? "✓ " : toast.type === "sync" ? "↻ " : toast.type === "quota" ? "⏱ " : "✕ "}
          {toast.message}
        </div>
      )}
    </div>
